	...
	function createTab(container, options, pp) {
		...
		pp.panel($.extend({}, options, {
			...
			onOpen: function(){
				...
				var popts = $(this).panel('options');
				state.selectHis.push(options.id);//标签记录修改为tabid，HSS，2016-12-12
	...
	function updateTab(container, param){
		...
		if (param.type == 'all' || param.type == 'header'){
			...
			if (opts.header){
                ...
                tab.attr('tabid',param.options.id);//header添加tabid属性，HSS，2016-12-12
			...
			if (oldTitle != opts.title){
				for(var i=0; i<selectHis.length; i++){
					if (selectHis[i] == oldTitle){
						selectHis[i] = opts.title;//这里暂时没修改，以后用到了再改，标签记录修改为tabid，HSS，2016-12-12
					}
				}
			}
		}
...
//		setScrollers(container);
		...
		for(var i=0; i<selectHis.length; i++){
			if (selectHis[i] == tabid){ //标签记录修改为tabid，HSS，2016-12-12
		...
		var hisTitle = selectHis[selectHis.length-1];//如果删除被选中的标签，选择上一次选中的tabs，否则保持被选择的tabs
		if (hisTitle){
	...
	function getTab(container, which, removeit){
		...
		}else if (which.indexOf('tabs_') == 0){//增加根据panel id获取tab,HSS,2016-12-12
            for(var i=0; i<tabs.length; i++){
                var p = tabs[i];
                if (p.panel('options').id == which){
                    tab = p;
                    if (removeit){
                        tabs.splice(i, 1);
                    }
    ...
    function getTabId(container){//获取当前选择标签的tabid，HSS，2016-12-12
        var currTab = getSelectedTab(container);
        if(currTab != undefined){
            return currTab.panel('options').id;
        }
        return 'tabs_0';
    }
	...
	$.fn.tabs.methods = {
        ...
        getTabId: function(jq){//获取当前选择标签的tabid，HSS，2016-12-12
            return getTabId(jq[0]);
        },
